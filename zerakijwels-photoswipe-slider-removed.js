...
var _abortController6, _onGestureChangedListener, _settledMedia, _onVariantChange, onVariantChange_fn, _onMediaChange, onMediaChange_fn, _onMediaSettle, onMediaSettle_fn, _onCarouselClick, onCarouselClick_fn, _getFilteredMediaIndexes, getFilteredMediaIndexes_fn, _onGestureStart, onGestureStart_fn, _onGestureChanged, onGestureChanged_fn;
var ProductGallery = class extends HTMLElement {
  ...
  openLightBox(index) {
    ...
    this._lightboxIndex = index ?? this.carousel.selectedIndex ?? 0;
    this.ensureCustomLightbox();
    this._customLightbox.classList.add("is-open");
    this.updateCustomLightbox();
    document.documentElement.style.overflow = "hidden";
  }
  ensureCustomLightbox() {
    if (!document.getElementById("custom-product-lightbox-styles")) {
      const style = document.createElement("style");
      style.id = "custom-product-lightbox-styles";
      style.textContent = `
    ...
    if (this._customLightbox) {
      return;
    }
    const modal = document.createElement("div");
    modal.className = "custom-product-lightbox";
    modal.innerHTML = `
      <div class="custom-product-lightbox__dialog" role="dialog" aria-modal="true" aria-label="Product images">
        <button type="button" class="custom-product-lightbox__close" aria-label="Close">&times;</button>
        <div class="custom-product-lightbox__content"></div>
      </div>
    `;
    document.body.appendChild(modal);
    modal.querySelector(".custom-product-lightbox__close").addEventListener("click", () => this.closeCustomLightbox());
    modal.addEventListener("click", (event) => {
      if (event.target === modal) {
        this.closeCustomLightbox();
      }
    });
    this._customLightboxKeydownHandler = (event) => {
      if (!this._customLightbox?.classList.contains("is-open")) {
        return;
      }
      if (event.key === "Escape") {
        this.closeCustomLightbox();
    ...
    document.addEventListener("keydown", this._customLightboxKeydownHandler);
    this._customLightbox = modal;
  }
  updateCustomLightbox() {
    if (!this._customLightbox || !this._lightboxImages?.length) {
      return;
    }
    const content = this._customLightbox.querySelector(".custom-product-lightbox__content");
    content.innerHTML = this._lightboxImages.map((image, imageIndex) => `
      <div class="custom-product-lightbox__item" data-lightbox-index="${imageIndex}">
        <img class="custom-product-lightbox__image" src="${image.currentSrc || image.src}" alt="${(image.alt || "").replace(/"/g, "&quot;")}">
      </div>
    `).join("");
    requestAnimationFrame(() => {
      const activeItem = content.querySelector(`[data-lightbox-index="${this._lightboxIndex}"]`);
      activeItem?.scrollIntoView({ block: "start", behavior: "auto" });
      this._customLightbox.scrollTop = Math.max(this._customLightbox.scrollTop - 68, 0);
    });
  }
  closeCustomLightbox() {
    this._customLightbox?.classList.remove("is-open");
...
if (window.location.pathname === '/pages/collectionspage') {
        document.querySelector('.section-spacing.bordered-section').style.display = 'none'
    }

document.addEventListener('DOMContentLoaded', () => {
    const bannerItems = document.querySelectorAll('.banner-image-item');

    if (bannerItems.length > 0) {
        bannerItems[0].addEventListener('click', () => {
            window.location.href = '/collections/bangles';
        });

        if (bannerItems[1]) {
            bannerItems[1].addEventListener('click', () => {
                window.location.href = '/collections/mangalsutra';
            });
        }
    }
});
