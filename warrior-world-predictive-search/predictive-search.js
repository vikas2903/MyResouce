...
class PredictiveSearchComponent extends Component {
  ...
  /**
   * Handles the full search form submit action, including the View all button.
  ...
  /**
   * Builds a search query that matches both normal product text and SKU codes.
   * @param {string} searchTerm - The raw term from the search input.
   * @returns {string}
   */
  #buildSearchQuery(searchTerm) {
    const escapedSearchTerm = searchTerm.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
    return `${searchTerm} OR variants.sku:"${escapedSearchTerm}"`;
  }

  /**
   * Redirects to the full search page using the combined product + SKU query.
   */
  #navigateToSearchResults() {
    ...
    searchUrl.searchParams.set('q', this.#buildSearchQuery(searchTerm));
  ...
  async #getSearchResults(searchTerm) {
    ...
    url.searchParams.set('q', this.#buildSearchQuery(searchTerm));
